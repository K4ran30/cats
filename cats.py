...
def report_progress(typed, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    num_correct = 0
    curr = {'id': user_id, 'progress': 0.0}
    for k in range(len(typed)):
        if typed[k] == prompt[k]:
            num_correct += 1
            curr['progress'] = num_correct/len(prompt)
        else:
            break
    upload(curr)
...
def fastest_words(match):
    ...
    for i in range(len(times)):
        print("Debug: Player Number ", i)
        fastest = fastest + [[]]
        for k in range(len(words)):
            print("Debug: Word ", words[k])
            if all([times[a][k] >= times[i][k] for a in range(len(times))]):
                if contains(words[k], fastest) == False:
                    fastest[i] = fastest[i] + [words[k]]
